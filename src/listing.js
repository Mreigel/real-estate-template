...
if (!container) {
  ...
  container.innerHTML = `
    <div class="max-w-6xl mx-auto px-4 py-8 space-y-10">
      ...
      <div class="overflow-x-auto flex gap-4 pb-4 scroll-smooth">
        ${prop.images.slice(1).map(src => `
          <img src="${src}" 
               class="h-48 min-w-[300px] object-cover rounded-lg shadow-md cursor-pointer"
               onclick="openLightbox('${src}')" />
        `).join('')}
      </div>

      <!-- Property Info -->
      <div class="bg-gray-800 p-8 rounded-2xl shadow-xl space-y-4 max-w-3xl mx-auto">
        <h1 class="text-3xl font-bold text-aquamarine">${prop.address}</h1>
        <p class="text-2xl text-white font-semibold">${prop.price}</p>
        <p class="text-base text-gray-300">${prop.description}</p>
        <p class="text-sm text-gray-500">Listed by: ${prop.seller}</p>

        <div class="pt-4">
          <a href="mailto:agent@example.com?subject=Interested in ${encodeURIComponent(prop.address)}" 
             class="px-6 py-3 bg-aquamarine text-gray-900 font-bold rounded-lg shadow hover:bg-opacity-90 transition">
            Contact Agent
          </a>
        </div>
      </div>
